...
- **Source categories** (host may match more than one; counts are unique hosts per category, alive public hosts):
  ...
  - **Tech** — fingerprint includes products that typically expose a login UI (CMS, Grafana, Kibana, GitLab/Gitea, Citrix, Keycloak, Jira, Confluence, …). Match the product as a word so `Atlassian Jira` counts as Jira.
