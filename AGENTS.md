...
- Before the tool runs, `run-host-scan.sh` does a **curl HTTP/1.1 GET** (15s max, same UA as the tool). One request records status and time (not two sequential GETs). Retry once on `000`.
- **robots** probes `{origin}/robots.txt`, not the site root (a slow/403 homepage must not skip a working robots.txt).
...
- **Nikto HTM** and **robots URL** are omitted when `skip_reason=host_unreachable` (no report / no Disallow list).
...
- **Software for expand:** `?software=` query wins; else fingerprint the row (Technologies tokens with version when present, title words so `Welcome to nginx!` matches nginx, web server, hostname labels — not only the first, so `uatapi.iis.cgi.com` matches IIS). Priority products include CMS, Kibana, Grafana, Prometheus, Elasticsearch, Jenkins, Kafka UI / Kafbat UI / AKHQ, Filebrowser, SonarQube, RabbitMQ, Redis Commander, Kiali, Kubecost, Superset, GitLab / Gitea / Gogs, Keycloak, Rancher, Argo CD (hostname `argocd` / `argo-*`), Eureka, Harbor, MinIO, Nexus, JFrog / Artifactory, Strapi, CloudBeaver, DBeaver, pgAdmin, UiPath, Control-M (title / hostname `control-m`), Oracle (hostname `oci-adb-control` / realm OCI ADB Control; not the bare `oci` cluster label), Tomcat, WildFly / JBoss, SharePoint, IIS, nginx, Apache, ASP.NET, PHP, Node.js, CrafterCMS, Java (not JavaScript). Named apps beat generic Java (SonarQube + Java → SonarQube). Not Cisco ASA VPN or Citrix NetScaler AAA. Technologies display drops bare Java when a named Java app is already listed, and aliases Wappalyzer `SonarQubes` to SonarQube.
- **nuclei only when a product is known** (filter or fingerprint). Do not offer nuclei for blind `-tags tech` with empty software. Backend `run-host-scan.sh` refuses nuclei when SOFTWARE is empty. Nuclei `output.txt` has no **Output:** filesystem path (Pass 1 and Pass 2 both use **Findings:**). Prefix real match lines with `[*]`; leave `No vulnerabilities discovered.` unprefixed. Blank line before `=== Pass 2`.
...
- **robots:** fetch `/robots.txt` (same idea as multiTabs → Directories in robots.txt). **txt** = raw `robots.txt` body; **url** = open Disallow directories in Firefox via `discover-robots:` → `misc/open-robots-tabs.sh` (only when `disallow_count` > 0). Skip `Disallow: /` (site root is not a directory to open). Keep query-string Disallow paths (Drupal `/?q=admin/` is not site root). Run does not open Firefox.
...
- **droopescan** / **wpscan** gate on CMS software (query or fingerprint). **WordPress → wpscan only** (no droopescan for WP). Drupal / Joomla / Moodle / Silverstripe still use droopescan when matched. Droopescan runs `--hide-progressbar`; report TXT drops percent-bar lines (`(0%)` / `12/4000`) and leading `[+]`, and writes **URLs found**.
...
- **By type** column **Type**: **Basic** (HTTP challenge: Basic / Digest / NTLM / Negotiate, including 401 with empty title) · **Form** (Path / Title / Tech, or WhatWeb PasswordField). Generic `401 Authorization Required` titles with HTTP-challenge do not count as Form. Host may be both. Links: `?login=basic|form`. PasswordField, PoweredBy, Index Of, Object (HTML tag), and header/protocol noise (`HSTS`, `HTTP/3`, `X-UA-Compatible`) stay out of Technologies / Software versions (`NOISE_PLUGINS`).
- **Source categories** (host may match more than one; counts are unique hosts per category, alive public hosts):
  - **Path** — newest `tools/host-scans/<host>/ffuf/*/ffuf.json` FUZZ path matches high-confidence login paths (`login`, `wp-login.php`, `oauth`, `sso`, …), **or** httpx `final_url` / `url` path (`/login.jsp`, …). Avoid bare `admin`. **SPA filter:** when a run has many hits sharing one body length (≥50 results, mode count ≥20, mode ≥50% of results), login-named paths with that same length are ignored (soft-200 app shell).
  - **Title** — httpx/page title matches login / sign-in / SSO / unauthorized / password phrases, plus common non-English titles (`Zaloguj`, `Anmelden`, `Connexion`, …).
  - **Tech** — fingerprint includes products that typically expose a login UI (CMS, Grafana, Kibana, GitLab/Gitea, Citrix, Keycloak, Jira, Confluence, Argo CD, Harbor, MinIO, Rancher, UiPath, pgAdmin, Control-M, …). Match the product as a word so `Atlassian Jira` counts as Jira. httpx tech tokens, page title, and hostname all count (SPA consoles often only have HSTS in tech).
