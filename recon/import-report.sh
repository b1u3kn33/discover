    ...
    if [ -f "$src/css/modern.css" ]; then
        ...
        if [ -f "$report/pages/subdomains.htm" ]; then
            sed -i \
                ...
                -e 's|inc-host-scan\.js?v=[0-9]*|inc-host-scan.js?v=57|g' \
...
need = [
    ...
    ("inc-host-scan.js", '<script src="../assets/javascript/inc-host-scan.js?v=57"></script>'),
]
...
# Always pin host-scan script to current cache-bust (robots tool, etc.).
import re
new_text, n = re.subn(
    r'inc-host-scan\.js\?v=[0-9]+',
    'inc-host-scan.js?v=57',
    text,
)
if n:
    text = new_text
